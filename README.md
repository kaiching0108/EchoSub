...
每段字幕會**自動即時寫入**專案目錄下的 `logs/` 資料夾，檔名帶時間戳：

```
logs/EchoSub_20260115_143022.txt
```

- 每次執行程式產生一個新檔（不覆寫歷史）。
- 內容為**雙語**：來源為英文/日文/韓文時寫「`[EN] 原文` ＋ 空一格＋中文翻譯」；
  來源為中文/粵語時只寫單行（已是繁體，不重複）。
- 編碼 **UTF-8（含 BOM）**，直接用 Windows 記事本開中文也正常。
- 關閉視窗即自動收尾，字幕不會遺失。

> 想改匯出位置：改 `config.py` 的 `EXPORT_DIR`（預設 `logs/`）。
